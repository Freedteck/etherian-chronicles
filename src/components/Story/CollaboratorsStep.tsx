...
const CollaboratorsStep = ({ data, errors, onUpdate }: CollaboratorsStepProps) => {
  const [newCollaborator, setNewCollaborator] = useState({ username: '', email: '' });

  const addCollaborator = () => {
    if (newCollaborator.username.trim() && newCollaborator.email.trim()) {
      const collaborator = {
        id: Date.now().toString(),
        username: newCollaborator.username.trim(),
        email: newCollaborator.email.trim()
      };
      onUpdate({
        collaborators: [...data.collaborators, collaborator]
      });
      setNewCollaborator({ username: '', email: '' });
    }
  };
      ...
      <Card className="p-8">
        ...
        <div className="space-y-6">
          <p className="text-sm text-muted-foreground">
            Invite other writers to collaborate on your story. Collaborators can add new chapters and help shape the narrative.
          </p>
          ...
          <div className="space-y-3">
            ...
            <Button
              ...
              disabled={!newCollaborator.username.trim() || !newCollaborator.email.trim()}
              className="w-full"
            >
      ...
      <Card className="p-8">
        ...
        <div className="space-y-4">
          <div className="bg-muted/30 rounded-lg p-4">
            <h3 className="font-semibold text-foreground mb-2">Community-Driven Story</h3>
            <p className="text-sm text-muted-foreground mb-3">
              Your story will be open to community collaboration. Readers can vote on chapter choices and story directions.
            </p>
            <ul className="text-xs text-muted-foreground space-y-1">
              ...
              <li>• Voting options from your first chapter will guide the next chapter</li>
            </ul>
          </div>
        </div>
      </Card>

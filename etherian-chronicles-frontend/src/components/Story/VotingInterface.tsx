...
const VotingInterface = ({ chapter, onVote, userVote = null }) => {
  const [selectedChoice, setSelectedChoice] = useState(userVote);
  ...
  useEffect(() => {
    const updateTimer = () => {
      const deadline = new Date(chapter.votingDeadline);
      const now = new Date();
  ...
  const handleVote = (choiceId) => {
    ...
    toast({
      title: "Vote Cast!",
      description: "Your vote has been recorded. The community decides the story's path.",
    });
  };

  const totalVotes = chapter.choices.reduce((sum, choice) => sum + choice.votes, 0);
    ...
    <div className="bg-card rounded-xl border border-border p-6">
      ...
      {chapter.resolved && (
        <div className="mb-6 p-4 bg-primary/10 border border-primary/20 rounded-lg">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            The community has chosen the winning path. The next chapter will continue this storyline.
          </p>
        </div>
      )}
      ...
      <div className="space-y-4">
        {chapter.choices.map((choice, index) => {
          ...
          const isWinner = chapter.resolved && chapter.winningChoice === choice.id;
          const isVotingOpen = !chapter.resolved && new Date() < new Date(chapter.votingDeadline);
          ...
          return (
            <div
              ...
              onClick={() => isVotingOpen && handleVote(choice.id)}
            >
              {/* Choice Content */}
